...
shared_contract=verify_contract()
CONTRACT=CONTRACT_DIR/'trinity-acceptance-snapshot.md'
...
for scope,key in [('required','required_predicates'),('optional','optional_predicates')]:
    assert {r['predicate'] for r in rows if r['scope']==scope}==set(shared_contract[key]),f'{scope} inventory differs from versioned contract'
parser=argparse.ArgumentParser(description=__doc__)
parser.add_argument('--check',action='store_true',help='Fail on generated-file drift without writing files')
args=parser.parse_args()
def emit(name,text):
    path=ROOT/name
    if args.check:
        if not path.exists() or path.read_text()!=text:raise SystemExit(f'Stale generated inventory: {name}; run predicate_inventory.py')
    else:path.write_text(text)
emit('predicate-inventory.json',json.dumps(rows,indent=2)+'\n')
required=sum(r['scope']=='required' for r in rows)
lines=['# ISOBASE predicate checklist','', 'Current reconciliation and non-predicate obligations: [STATUS.md](STATUS.md). Historical audit batches below are not current pass counts.','',f'The acceptance matrix lists **{required} required callable predicate/arities** and one optional extension. Arity ranges are expanded below.','',
